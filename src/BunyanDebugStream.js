...
const LEVELS = (function() {
  const answer = {};
  const o = (level, prefix, colors) => answer[level] = {
    level,
    prefix,
    colors
  };
...
class BunyanDebugStream extends Writable {
  ...
  constructor(options) {
    ...
    if (this.options.stringifiers != null) {
      for (let key in this.options.stringifiers) {
    ...
    try {
      if ( (stringifier == null) ) {
        ...
        if ( (result == null) ) {
          ...
          for (key of Array.from((result.consumed != null ? result.consumed : []))) {
            consumed[key] = true;
          }
...
${this._indent}`);
  ...
  _entryToString(entry) {
    ...
    const consumed = {};
    for (let field of Array.from(FIELDS_TO_IGNORE)) {
      consumed[field] = true;
    }
    ...
    let line = `${date}${processStr}${levelPrefix}${src}${prefixes}${applyColors(message, colorsToApply)}`;
